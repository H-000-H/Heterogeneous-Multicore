...
#define SPI_CONTROLLER_COUNT DTC_GEN_COUNT_ESP32_SPI
...
static int spi_controller_probe(struct device* dev)
{
    ...
    priv->host_id  = host;
    priv->host     = hal_spi_bus_host_get(host);
    if (!priv->host) {
    ...
    if (bus_controller_bind(dev, &spi_bus_type, priv->host) != 0) {
        osal_pool_release(s_controller_used, SPI_CONTROLLER_COUNT, pool_idx);
        (void)hal_spi_bus_host_deinit(host);
        return VFS_ERR_IO;
    }
    ...
    SYS_LOGI(kTag, "controller probe OK: host=%d mosi=%d miso=%d sclk=%d children=%d",
             host, mosi, miso, sclk, child_count);
    return VFS_OK;
