...
/*从设备初始化*/
static int spi_slave_hw_init(struct hal_spi_bus_host* host,const struct hal_spi_device_config* dev_cfg)
{
    if (!host || !dev_cfg)
        return VFS_ERR_INVAL;
    if (host->hw_inited)
...
/*数据扔进缓冲区(传输前必须使用)*/
static int spi_slave_setup_trans(struct hal_spi_ctx* ctx, struct hal_spi_hw* hw,size_t len, const uint8_t* tx, uint8_t* rx_buf)
{
...
/*添加主设备*/
static int spi_master_device_add(struct hal_spi_ctx* ctx, struct hal_spi_hw* hw,const struct hal_spi_device_config* dev_cfg)
{
...
static int spi_master_hw_init(struct hal_spi_bus_host* host,struct hal_spi_ctx* ctx,struct hal_spi_hw* hw,const struct hal_spi_device_config* dev_cfg)
{
...
int hal_spi_interface_attach(struct hal_spi_ctx* ctx)
{
    ...
    if (host->hw_inited &&
        (!hal_pin_equal(host->active_cfg.cs_pin, ctx->cfg.cs_pin) ||
...
int hal_spi_bus_reconfigure(struct hal_spi_bus_host* host,const struct hal_spi_device_config* dev_cfg)
{
...
/*设备上下文初始化*/
void hal_spi_ctx_init(struct hal_spi_ctx* ctx, int pool_idx,struct hal_spi_bus_host* host,const struct hal_spi_device_config* dev_cfg)
{
    int hw_idx;
    struct hal_spi_hw* hw;
