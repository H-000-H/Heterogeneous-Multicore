...
/*===========================================================================================================================================================*/
                                                            ...
                                                            /*DMA 实体与 API*/
/*===========================================================================================================================================================*/
typedef void (*hal_dma_callback_t)(struct hal_dma_chan* chan, void* user_data);

struct hal_dma_chan
{
    int (*init)(struct hal_dma_chan* chan, const struct hal_dma_config* cfg);
    int (*config)(struct hal_dma_chan* chan, const void* src, void* dst, size_t len);
    int (*start)(struct hal_dma_chan* chan);
    int (*stop)(struct hal_dma_chan* chan);
    int (*reset)(struct hal_dma_chan* chan);
    int (*deinit)(struct hal_dma_chan* chan);
    int (*set_callback)(struct hal_dma_chan* chan, hal_dma_callback_t cb, void* user_data);
    int (*busy)(struct hal_dma_chan* chan);      /* 查询传输是否完成: 0 = 空闲, 1 = 繁忙 */
    void* _impl;
};
...
static inline int hal_dma_config_block(struct hal_dma_chan* chan,
