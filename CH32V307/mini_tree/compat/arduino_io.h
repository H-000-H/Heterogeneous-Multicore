 ...
 *
 * 基础用法（需要已获取 struct device*）:
 *   struct device* led = device_find_by_label("led0");
...
/* 配置引脚模式 */
static inline void pinMode(struct device* dev, hal_pin_t pin, int mode)
{
...
/* 设置引脚电平 */
static inline void digitalWrite(struct device* dev, hal_pin_t pin, int val)
{
    struct gpio_level_arg arg;
    arg.pin   = pin;
    arg.level = val;
...
/* 读取引脚电平 */
static inline int digitalRead(struct device* dev, hal_pin_t pin)
{
    struct gpio_level_arg arg;
    arg.pin   = pin;
    arg.level = 0;
