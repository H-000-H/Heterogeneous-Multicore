...
```
board/dts/ch32v307.dts             板级入口 (/dts-v1/, includes, / { }, &label)
board/dtsi/ch32v307.dtsi           SoC 根 / { compatible, cpus, soc: soc { ... } }
board/dtsi/ch32v307-spi.dtsi       IP: #include soc.dtsi + &soc { spi@0 { ... } }
...
| Linux 内核 | mini_tree (CH32) |
|------------|------------------|
| `ch32v307.dtsi` | `board/dtsi/ch32v307.dtsi` |
| `ch32v307-evb.dts` | `board/dts/ch32v307.dts` |
| `&soc { ... };` | 同左 |
...
见 `board/dtsi/ch32v307-spi.dtsi`：

1. `#include "ch32v307.dtsi"`
...
| SoC | `board/dtsi/ch32v307.dtsi` | `cpus` + `soc: soc { simple-bus }` |
| IP 模板 | `board/dtsi/ch32v307-spi.dtsi` 等 | `&soc { dev@reg }` |
| 板级 | `board/dts/ch32v307.dts` | includes + `/ { }` + `&label` |
...
SPI 模板见 `board/dtsi/ch32v307-spi.dtsi`（默认 disabled）。启用示例：
...
## compatible: `ch32,spi-host` / `ch32,spi-device`

Bus Host + Interface 两级节点。模板见 `board/dtsi/ch32v307-spi.dtsi`；常量见 `board/dt-bindings/spi/spi-parameter.h`。
