...
| HAL (CPU / GPIO / SPI / …)  | `components/mini_tree/hal_if/src/hal_esp32s3.c` 等   |
...
### 3.5 `components/mini_tree/hal_if/`, `hal_bus/`, `hal_inst/` — HAL 三件套
- **`hal_if` (interface)**：平台相关 **API 接口** + **平台实现**。例如 `hal_pulse_engine.h` + `hal_pulse_engine_esp32s3.c`（RMT 编码器）。
- **`hal_bus` (bus abstraction)**：总线型设备抽象（`hal_spi_bus.h` 定义 `struct hal_spi_bus { write, read, write_top_half }`），平台无关的 vtable。
- **`hal_inst` (instance)**：具体芯片实例实现（`hal_spi.c` 把 ESP-IDF `spi_slave_*` 包成 HAL 语义）。
...
| HAL 总线            | `hal_inst/src/hal_spi.c`                              | `hal_spi_bus_host_init / xfer_begin/end` |
| HAL Bus vtable      | `hal_bus/include/hal_spi_bus.h`                       | `struct hal_spi_bus { write/read/... }` |
| HAL Bus host 状态   | `hal_inst/include/hal_spi_bus_host.h`                 | `struct hal_spi_bus_host`               |
...
- HAL SPI 实现: [hal_inst/src/hal_spi.c](file:///d:/Heterogeneous-Multicore-project/ESP32-S3/components/mini_tree/hal_inst/src/hal_spi.c)
