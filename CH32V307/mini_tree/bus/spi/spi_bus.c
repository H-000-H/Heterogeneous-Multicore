...
static int spi_bus_ops_transfer(void* ctx, const void* tx, void* rx,
...
static int spi_bus_ops_write(void* ctx, const void* data, size_t len,
                             uint32_t timeout_ms)
{
    struct spi_bus_client* cli = (struct spi_bus_client*)ctx;
    if (!cli || !cli->dev)
...
static int spi_bus_ops_read(void* ctx, void* data, size_t len,
                            uint32_t timeout_ms)
{
    struct spi_bus_client* cli = (struct spi_bus_client*)ctx;
    if (!cli || !cli->dev)
