...
extern GPIO_TypeDef *const g_ch32_port_lut[HAL_GPIO_PORT_COUNT];
extern const uint16_t      g_ch32_pin_lut[HAL_GPIO_PIN_COUNT];
...
static inline int COMPAT_WARN_UNUSED_RESULT hal_gpio_fast_set_level(hal_pin_t pin, int level)
{
    ...
    GPIO_TypeDef *port     = g_ch32_port_lut[port_idx];

    if (level)
        port->BSHR = g_ch32_pin_lut[pin_idx];
    else
        port->BCR = g_ch32_pin_lut[pin_idx];
    return VFS_OK;
}
...
static inline int COMPAT_WARN_UNUSED_RESULT hal_gpio_fast_get_level(hal_pin_t pin, int *level_out)
{
    ...
    *level_out = (port->INDR & g_ch32_pin_lut[pin_idx]) ? 1 : 0;
    return VFS_OK;
}
...
static inline int COMPAT_WARN_UNUSED_RESULT hal_gpio_fast_toggle(hal_pin_t pin)
{
    ...
    port->OUTDR ^= g_ch32_pin_lut[pin_idx];
    return VFS_OK;
}
/*===========================================================================================================================================================*/
