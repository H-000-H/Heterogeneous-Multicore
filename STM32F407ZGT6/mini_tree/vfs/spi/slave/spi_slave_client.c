...
static int spi_xfer_session_end(struct hal_spi_dev* ctx, int io_ret)
{
...
static int spi_slave_open(struct device* pdev, void* arg)
{
    ...
    if (first == 1)
    {
        ret = hal_spi_dev_hw_open(&priv->spi_dev);
        if (ret != VFS_OK)
            dev_lc_open_abort(lc);
        else
            priv->spi_dev.hw_open = 1;
    }
...
static int spi_slave_close(struct device* pdev)
{
    ...
    if (last && priv->spi_dev.hw_open)
    {
        COMPAT_IGNORE_RESULT(hal_spi_dev_hw_close(&priv->spi_dev));
        priv->spi_dev.hw_open = 0;
    }
...
static int spi_slave_write(struct device* pdev, const void* buffer, size_t len, uint32_t timeout_ms)
{
    ...
    ret = hal_spi_xfer_begin(&priv->spi_dev, timeout_ms);
    if (ret == VFS_OK)
    {
        int write_bytes = priv->spi_dev.host->dev.ops->write(hal_spi_host_bus(priv->spi_dev.host),
        ...
        ret = spi_xfer_session_end(&priv->spi_dev, ret);
    }
...
static int spi_slave_read(struct device* pdev, void* buffer, size_t len, uint32_t timeout_ms)
{
    ...
    if (ret == VFS_OK)
    {
        ret = priv->spi_dev.host->dev.ops->read(hal_spi_host_bus(priv->spi_dev.host), (uint8_t*)buffer, len);
        ret = spi_xfer_session_end(&priv->spi_dev, ret);
    }
...
static int spi_slave_ioctl(struct device* dev, int cmd, void* arg, size_t arg_len, uint32_t timeout_ms)
{
    ...
    case SPI_CMD_READ:
    {
        ...
        else
        {
            ...
            if (ret == VFS_OK)
            {
                ret = priv->spi_dev.host->dev.ops->read(hal_spi_host_bus(priv->spi_dev.host), ra->data, ra->len);
                ret = spi_xfer_session_end(&priv->spi_dev, ret);
            }
        }
        break;
    ...
    case SPI_CMD_QUEUE_TX:
    {
        ...
        if (!qa || arg_len != sizeof(*qa) || !qa->data || qa->len == 0)
            ret = VFS_ERR_INVAL;
        else if (!hal_spi_bus_supports_async_tx(hal_spi_host_bus(priv->spi_dev.host)))
            ret = VFS_ERR_IO;
        else
        {
            ret = hal_spi_xfer_begin(&priv->spi_dev, timeout_ms);
            if (ret == VFS_OK)
            {
                ret = hal_spi_bus_queue_tx(hal_spi_host_bus(priv->spi_dev.host), qa->data, qa->len);
                ret = spi_xfer_session_end(&priv->spi_dev, ret);
            }
        }
        break;
    ...
    case SPI_CMD_GET_TRANS_RESULT:
    {
        ...
        if (!tra || arg_len != sizeof(*tra))
            ...
            ret = hal_spi_get_trans_result(&priv->spi_dev, tra->data, tra->len,
                                           tra->trans_len, timeout_ms);
        break;
    }
    case SPI_CMD_DEINIT:
        if (priv->spi_dev.hw_open)
        {
            COMPAT_IGNORE_RESULT(hal_spi_dev_hw_close(&priv->spi_dev));
            priv->spi_dev.hw_open = 0;
        }
...
int spi_slave_client_probe(struct device* pdev)
{
    ...
    hal_spi_dev_init(&priv->spi_dev, pool_idx, bus_host, &dev_cfg);
    hal_spi_dev_register(&priv->spi_dev);
...
err_pool:
    hal_spi_dev_unregister(&priv->spi_dev);
    __builtin_memset(priv, 0, sizeof(*priv));
