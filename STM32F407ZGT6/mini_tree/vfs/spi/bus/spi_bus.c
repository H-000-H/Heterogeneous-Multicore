/* STM32 — SPI 总线 probe（仅主机模式；从机见 ESP32 树） */
#include "bus.h"
#include "hal_spi.h"
#include "device.h"
#include "driver.h"
...
#ifndef DTC_GEN_COUNT_STM32_SPI_MASTER
#define DTC_GEN_COUNT_STM32_SPI_MASTER 0
#endif
#ifndef DTC_GEN_COUNT_STM32_SPI_HOST
#define DTC_GEN_COUNT_STM32_SPI_HOST 0
#endif

#define SPI_CONTROLLER_COUNT \
    (DTC_GEN_COUNT_STM32_SPI_MASTER + DTC_GEN_COUNT_STM32_SPI_HOST)
...
static int spi_master_controller_probe(struct device* dev)
{
    ...
    bus_cfg.bus_role        = HAL_SPI_BUS_ROLE_MASTER;
    ...
    SYS_LOGI(kTag, "master probe OK: host=%d mosi=%d:%d miso=%d:%d sclk=%d:%d",
             host,
             (int)HAL_PIN_PORT(mosi), (int)HAL_PIN_NUM(mosi),
             (int)HAL_PIN_PORT(miso), (int)HAL_PIN_NUM(miso),
             (int)HAL_PIN_PORT(sclk), (int)HAL_PIN_NUM(sclk));
    return VFS_OK;
...
DRIVER_REGISTER(stm32_spi_master_bus, "stm32,spi-master", spi_master_controller_probe, spi_controller_remove)
DRIVER_REGISTER(stm32_spi_host_legacy, "stm32,spi-host", spi_master_controller_probe, spi_controller_remove)
