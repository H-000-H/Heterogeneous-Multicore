                               ...
                               uint8_t* rx, size_t len, uint32_t timeout_ms)
{
    ...
    return hal_spi_transfer(&client->spi_dev, tx, rx, len, timeout_ms);
}
...
int spi_master_client_hw_open(struct spi_master_client* client)
{
    ...
    ret = hal_spi_dev_hw_open(&client->spi_dev);
    if (ret == VFS_OK)
        client->spi_dev.hw_open = 1;
...
{
    if (!client || !client->spi_dev.hw_open)
        return;

    COMPAT_IGNORE_RESULT(hal_spi_dev_hw_close(&client->spi_dev));
    client->spi_dev.hw_open = 0;
}
                           ...
                           int pool_idx)
{
    ...
    hal_spi_dev_init(&client->spi_dev, pool_idx, bus_host, &dev_cfg);
    hal_spi_dev_register(&client->spi_dev);

    if (osal_mutex_create_static(&client->io_mutex, mutex_storage,
                                 mutex_storage_size) != 0)
    {
        hal_spi_dev_unregister(&client->spi_dev);
        __builtin_memset(client, 0, sizeof(*client));
        return VFS_ERR_IO;
    }
